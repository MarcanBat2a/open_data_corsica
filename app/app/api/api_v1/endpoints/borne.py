from app.app.services import borne_recharge_services
...
@router.get("/")
def read_bornes():
    return borne_recharge_services.get_all_borne()
...
def export_comment(extension:str):
    ...
    if extension.lower()=="csv":
        stream = io.StringIO()
        dataframe_result = pd.DataFrame(data=borne_recharge_services.get_all_borne())
        ...
        stream = io.StringIO()
        result = json.dump(borne_recharge_services.get_all_borne_to_dict(), stream)
        response = StreamingResponse(iter([stream.getvalue()]),
